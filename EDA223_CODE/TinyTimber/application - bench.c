...
void BENCHMARK_TONEGEN(){          
	char buf [100]; 
	Time d, avg=0, max = 0;
	for (int i = 0; i < 500; i++) { 
		Time t1 = CURRENT_OFFSET();
		for (int j=0; j<1000; j++){ //have to scale up, d = 10 every some time due to delay in startup time, existence of cache would make it even bigger
			if (*DAC_SOUND_REG) {
				*DAC_SOUND_REG = 0;
			} else {
				*DAC_SOUND_REG= tonegen.volume;
			}
		}
	...
	d = avg; 
	avg /= 500;
	sprintf(buf, "max = %ld, sum=%ld, avg = %ld [nsec]\n", USEC_OF(max), USEC_OF(d), USEC_OF(avg)); 
	SCI_WRITE(&sci0, buf);
}
...
void BENCHMARK_LOAD(){          
	...
	for (int i = 0; i < 500; i++) { 
		Time t1 = CURRENT_OFFSET();
		for (int i=0; i<backgrl.background_loop_range; i++);
		Time t2 = CURRENT_OFFSET();
		d = (t2 - t1) ;
		if (d > max) max = d; 
		avg += d;
	...
	avg /= 500;
	char buf[100];
	sprintf(buf, "max = %ld, sum=%ld, avg = %ld [usec]\n", USEC_OF(max), USEC_OF(d), USEC_OF(avg)); 
	SCI_WRITE(&sci0, buf);
}	
...
void benchmark (App* self, int arg) {
	if (self->isBenchRunning) {
		...
		if (arg == 0){
			...
			BENCHMARK_TONEGEN();
					self->isBenchRunning = 0;

		} else if (arg == 1){
								self->isBenchRunning = 1;

			SCI_WRITE(&sci0, "Benchmark background load = ");
			sprintf(buf, "%d\n", backgrl.background_loop_range);
			SCI_WRITE(&sci0, buf);
			...
			backgrl.background_loop_range = 13500;
									self->isBenchRunning = 1;
